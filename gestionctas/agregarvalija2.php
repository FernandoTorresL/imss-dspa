<?php
  ...
  if (isset($_POST['submit'])) {
    ...
    if (!empty($num_oficio_ca) && !empty($num_oficio_del) && 
      ...
      echo 'Tama&ntilde;o:' . $new_file_size . '<br />';
        ...
        if ($_FILES['new_file']['error'] == 0) {
          ...
          if (move_uploaded_file($_FILES['new_file']['tmp_name'], $target)) {
            ...
            $query = "INSERT INTO valijas 
              ...
              id_remitente, delegacion, comentario, archivo)
              VALUES 
              ( '$num_oficio_ca', '$num_oficio_del', 
                '$fecha_recepcion_ca', NOW(), '$fecha_valija_del', 
                0, '$cmbDelegaciones', '$comentario', '$timetime $new_file' )";
            //echo $query;

            mysqli_query($dbc, $query);

            // Confirm success with the user
            echo '<p class="nota"><strong>La nueva valija ha sido creada exitosamente. </strong>Regresa al <a href="indexCuentasSINDO.php">inicio</a></p>';
  ...
  <p>Por favor captura los datos solicitados para crear una nueva valija.</p>
    <form enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
      <fieldset>
        ...
        <label for="fecha_recepcion_ca">Fecha recepci&oacute;n:</label>
